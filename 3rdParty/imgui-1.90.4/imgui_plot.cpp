        ...
        if (conf.selection.show) {
            if (hovered) {
                if (g.IO.MouseClicked[0]) {
                    ...
                    const int v_idx = cursor_to_idx(g.IO.MousePos, inner_bb, conf, x_min, x_max);
                    int32_t start = conf.values.offset + (v_idx % conf.values.count);
                    int32_t end = start;
            ...
            if (g.ActiveId == id) {
                if (g.IO.MouseDown[0]) {
                    const int v_idx = cursor_to_idx(g.IO.MousePos, inner_bb, conf, x_min, x_max);
                    const int32_t start = *conf.selection.start;
                    int32_t end = conf.values.offset + (v_idx % conf.values.count);
                    if (end > start) {
                        if (conf.selection.sanitize_fn)
                            end = conf.selection.sanitize_fn(end - start) + start;
                        if (end < conf.values.offset + conf.values.count) {
